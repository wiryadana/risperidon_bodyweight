...
#### Data Analysis
This study utilized the standardized mean difference (SMD) as the primary effect measure. Random effect model utilized to find the pooled standardized mean difference across two groups (Risperidon vs non-risperidon or control). If the published data incomplete, we primarily contact the corresponding author to kindly provide the data. Second measures was to calculate from the available data in published papers. If the study provide mean and confidence interval (CI), we converted the CI to standard deviation (SD) through standard error using methods provided by Cochrane handbook [@higgins_cochrane_2021]. All data preparation and analysis was conducted in R v4.1.2 and Rstudio server v.14 for GNU/Linux Ubuntu server 21.04 [@r-base;@rstudio_team_rstudio_2021;@canonical_ubuntu_2021]. Additional data processing and analysis, including general data wrangling conducted with Tidyverse package [@tidyverse], data retrieval with [@googlesheets4] and table generation with [@DT;@gtsummary]. Meta-analysis specific calculation including inverse variant method and generation of forest and funnel plot conducted with Meta package [@meta]. PRISMA 2020 chart created with PRISMA2020 package [@PRISMA2020].
